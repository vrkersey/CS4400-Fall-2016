...
/* A function provided to make_dictionary() that
   is used to destroy each values in the dictionary when
   the dictionary itself is destroyed or when the
   value is replaced with another value: */
...
/* Creates a dictionary with the given comparion mode
   and value-destruction function: */
...
/* Sets the dictionary's mapping for `key` to `value`.
   The dictionary makes its own copy of `key` and does not
   refer to that pointer on return. It keeps the `value`
   pointer as-is. */
void dictionary_set(dictionary_t *d, const char *key, void *value);

/* Returns the dictionary's value for `key`, or NULL if the
   dictionary has no value for `key`: */
